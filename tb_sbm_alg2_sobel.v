...
//   4) 协议：tuser 仅帧首拍、tlast 仅行末拍；每帧 IMG_W×IMG_H 个输出，
//      无气泡、无丢拍；
...
sbm_alg2_sobel #(.IMG_W(IMG_W), .IMG_H(IMG_H)) u_dut (
	.clk(clk), .rst_n(rst_n),
	...
	begin
		for (r = 0; r < IMG_H; r = r + 1) begin
			for (c = 0; c < IMG_W; c = c + 1) begin
				// 必须 negedge 驱动：若与 DUT 采样同沿(posedge)做 NBA 更新，
				// 会因活动区顺序竞态采到旧值，造成像素重复消费/流错位
				@(negedge clk);
